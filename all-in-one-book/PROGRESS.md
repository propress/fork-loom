...
1. **项目定位** (1 段)
   - Loom 是什么：AI 驱动的编程助手，Rust 实现，99 个 crate
   - 核心特点：模块化、服务端 LLM 代理、远程执行环境（Weaver）
   - 与类似项目的区别：强调安全（API Key 永不在客户端）、状态机驱动、IoC 架构

2. **架构全景图** (1 张 Mermaid 图 + 文字解释)
   - 画出 Client (loom-cli) ↔ Server (loom-server) ↔ External Services (Anthropic/OpenAI/K8s) 的三层架构
   - 标注核心模块：loom-common-core (状态机)、loom-server-llm-service (LLM 代理)、loom-server-weaver (K8s 编排)
   - 文字逐步解释图中每个节点的职责

3. **核心概念词典** (表格)
   - 从上面"术语约定"中选出最核心的 10 个术语（Thread、Weaver、Tool、Agent、ProxyLlmClient、LlmService、PostToolsHook、ConversationContext、ToolExecutionStatus、Spool）
   - 每个术语：一句话定义 + 类比标准概念 + 关键区别

4. **代码库地图** (树状结构 + 说明)
   - 列出 crates/ 下的主要分类（common-*、server-*、cli-*、tui-*、llm-*、auth-*、analytics-*、weaver-*、wgtunnel-*）
   - 每个分类 2-3 句话说明用途
   - 标注"起点 crate"（初学者应该从哪里开始看）：loom-common-core、loom-cli、loom-server

5. **一次典型交互的极简全流程** (序列图 + 文字)
   - 场景：用户在 loom-cli 输入 "帮我读取 main.rs 文件"
   - 画序列图：User → CLI → Agent (状态机) → LLM (通过 ProxyLlmClient) → Server (LlmService) → Anthropic API → 返回 tool_use → Agent → ToolRegistry → read_file → 返回结果 → LLM → 最终回复
   - 文字逐步解释每一跳发生了什么
   - **重点**：数据在每一跳的形态变化（Message → LlmRequest → HTTP JSON → SSE Stream → LlmEvent → AgentEvent → ToolCall → ToolExecutionStatus → Message）

#### 写作时注意

- 序言的目标是"**让读者快速建立全局认知**"，不是教学
- 流程图和序列图的粒度：**模块级**，不要画函数级
- 复杂节点标注"详见第 N 章"，不要在序言展开
- 极简全流程：只跟踪"happy path"（成功路径），不讨论错误处理和重试
- 每张图下方必须有文字逐步讲解，图和文字要完全对应

#### 质检重点

- [ ] 是否包含了 5 个强制部分
- [ ] 全景图是否清晰（一眼能看懂三层架构）
- [ ] 典型交互流程是否完整（用户输入 → 最终回复，中间每一跳都标注了）
- [ ] 数据形态变化是否讲清楚了（每一跳数据的类型是什么）
- [ ] 代码片段是否不超过 3 处（序言应该几乎没有代码）
- [ ] 术语是否都有首次解释
- [ ] 过渡是否自然（5 个部分之间的衔接）

### 待验证项

- 无（序言基于已阅读的规范文档和代码，无需额外验证）
...
- **序言中代码片段应为 0**（最多只能有类型定义，且不超过 5 行）
- 用调用路径 + 文字描述代替代码
- 极简全流程用序列图，不用代码
...
- 图中每个节点必须对应实际存在的模块
- 图下方必须有文字逐步解释
- 粒度控制：全景图 = 模块级，不混入函数级细节

### 时间控制

- 序言预计 30-40 分钟完成（包含质检）
- 如果时间不够，优先完成前 4 部分（项目定位、全景图、概念词典、代码库地图），极简全流程可以留给下次

---

**下次执行时**：直接从这里开始写 ch00-preface.md，按照上述交接备忘的 5 个强制部分逐一完成。
